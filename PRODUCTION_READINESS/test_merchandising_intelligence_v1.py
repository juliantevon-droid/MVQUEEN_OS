...
class MerchandisingIntelligenceV1Tests(unittest.TestCase):
    ...
    def ready_catalog(self):
        return [
            run(self.raw("M1", "Necklace", "Black")),
            run(self.raw("M2", "Earrings", "Black")),
            run(self.raw("M3", "Necklace", "Black", material="Sterling silver")),
            run(self.raw("P1", "Bracelet", "Pink")),
        ]

    def test_resolver_is_brand_world_safe_and_schema_valid(self):
        resolved = resolve_catalog(self.ready_catalog())
    ...
    def test_resolver_is_deterministic(self):
        first = resolve_catalog(self.ready_catalog())
        second = resolve_catalog(self.ready_catalog())
