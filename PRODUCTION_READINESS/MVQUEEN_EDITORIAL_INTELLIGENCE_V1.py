...
Deterministic, category-aware copy generation. It only uses verified source facts
as factual inputs and rejects unsupported/high-risk claims and generic filler.
"""
...
FORBIDDEN_COPY_RE = re.compile(r"\b(cures?|treats?|prevents?|guaranteed?|clinically proven|medical[- ]grade|hypoallergenic|non[- ]toxic|chemical[- ]free|organic|certified|#1|100%|instant|permanent)\b", re.I)
GENERIC_RE = re.compile(r"\b(perfect for any occasion|versatile and stylish|must[- ]have|game[- ]changer|elevate your everyday|elevate your wardrobe|level up|designed to elevate|whether you'?re dressing up or down)\b", re.I)
...
def verified_facts(record: Dict[str, Any]) -> Dict[str, Any]:
    return {_text(f.get("name")).lower(): f.get("value") for f in record.get("source_truth", {}).get("facts", []) if f.get("verified") is True and _text(f.get("name"))}
...
def _detail(facts: Dict[str, Any], category: str) -> str:
    material = _safe(_fact(facts, "material", "fabric"))
    color = _safe(_fact(facts, "color", "shade"))
    finish = _safe(_fact(facts, "finish"))
    texture = _safe(_fact(facts, "texture"))
    size = _safe(_fact(facts, "size", "dimensions"))
    ingredient = _safe(_fact(facts, "ingredient", "key_ingredient"))
...
def generate(record: Dict[str, Any]) -> Dict[str, Any]:
    ...
    product_type = _safe(record.get("category", {}).get("product_type")) or "piece"
    category = classify_category(product_type)
    detail = _detail(facts, category)
    use = _safe(_fact(facts, "use_context", "usage", "occasion")) or "her everyday routine"
    color = _safe(_fact(facts, "color", "shade"))
    material = _safe(_fact(facts, "material", "fabric"))
    finish = _safe(_fact(facts, "finish"))
    ingredient = _safe(_fact(facts, "ingredient", "key_ingredient"))
    ...
    if category == "fashion":
        titles = [f"{color + ' ' if color else ''}{product_type}", f"The {color.lower() + ' ' if color else ''}{product_type.lower()}", f"{product_type} — {color}" if color else f"{product_type} — MVQueen"]
        openings = [
        ...
        titles = [f"{product_type} for Her Routine", f"The {product_type}", f"{product_type} — MVQueen"]
        openings = [
        ...
        titles = [f"{product_type} for the MVQueen Look", f"The {product_type}", f"{product_type} — MVQueen"]
        openings = [
        ...
        titles = [f"{color + ' ' if color else ''}{product_type}", f"The {product_type}", f"{product_type} — MVQueen"]
        openings = [
        ...
        openings = [f"A considered {product_type.lower()} for {use.lower()}, with a polished point of view.", f"This {product_type.lower()} keeps the experience simple, modern, and confidence-led.", f"For her everyday moments, this {product_type.lower()} brings an intentional finish to the way she chooses and uses what belongs in her world."]
        closer = "Simple, polished, and unmistakably personal."

    title = _choose(titles, record, "title")
    opening = _choose(openings, record, "opening")
    details: List[str] = []
    if material: details.append(f"Made with {material}.")
    if color: details.append(f"Available in {color}.")
    if ingredient: details.append(f"Ingredient listed: {ingredient}.")
    if finish: details.append(f"Finish: {finish}.")
    if not details and detail: details.append(f"Product detail: {detail}.")

    description = " ".join([opening, " ".join(details), closer]).strip()
    benefits = []
    if material: benefits.append(f"The listed {material.lower()} construction supports the intended styling direction.")
    if color: benefits.append(f"The {color.lower()} color direction makes styling decisions easy to build around.")
    if finish: benefits.append(f"The listed {finish.lower()} finish helps define the final look.")
    if ingredient: benefits.append(f"Includes {ingredient} as a listed ingredient.")
    if not benefits: benefits.append("Clear product details support a more confident, informed selection.")

    return {"title": title, "short_description": opening, "description": description, "benefits": benefits, "features": details or [f"Product type: {product_type}."], "cta": "Shop MVQueen", "_editorial_category": category}
...
def validate_editorial(record: Dict[str, Any]) -> Tuple[List[str], List[str]]:
    ...
    if PLACEHOLDER_RE.search(text): errors.append("Editorial placeholder/template language detected")
    if FORBIDDEN_COPY_RE.search(text): errors.append("Unsupported or high-risk claim language detected in editorial copy")
    if GENERIC_RE.search(text): errors.append("Generic/robotic marketing phrase detected in editorial copy")
    facts = verified_facts(record)
    values = [str(v).strip().lower() for v in facts.values() if _text(v)]
    if values and not any(v in text.lower() for v in values): errors.append("Customer-facing copy contains no verified product-specific detail")
    if not values: errors.append("No verified product facts available for distinctive customer-facing copy")
    if len(text.split()) < 18: warnings.append("Editorial copy is short; consider richer verified source data")
    return errors, warnings

__all__ = ["generate", "validate_editorial", "verified_facts", "classify_category"]
