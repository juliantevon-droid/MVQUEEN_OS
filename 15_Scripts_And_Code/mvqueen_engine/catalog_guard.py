...
PROTECTED_FIELDS = {
    "Handle", "Product ID", "Variant ID", "SKU", "Variant SKU",
    "Inventory", "Inventory Qty", "Option1 Name", "Option1 Value",
    "Option2 Name", "Option2 Value", "Option3 Name", "Option3 Value",
    "Variant Inventory Qty", "Variant Inventory Tracker",
    "Variant Inventory Policy", "Variant Fulfillment Service",
    "Variant Requires Shipping", "Variant Taxable", "Barcode",
...
def validate_row(row: Dict[str, str]) -> List[str]:
    ...
    text = " ".join(
        str(row.get(k, ""))
        for k in ("Title", "Body HTML", "Product Type", "Tags", "SEO Title", "SEO Description")
    )
    if CONTAMINATION.search(text):
        issues.append("supplier-contamination")
    return issues

def validate_protected(before: Dict[str, str], after: Dict[str, str]) -> List[str]:
    return [
        f"protected-mutated:{k}"
