...
Load this file when initializing any AI session to calibrate voice before generating content.
...
She sprays it before she is ready. That is the ritual. A scent that settles into skin like a decision — warm, deliberate, unmistakably hers.

### Product Description — Jewelry
Delicate does not mean small. This piece sits quietly until the room notices. Wear it like you already knew it would.

### Caption — Empowerment
She is not waiting for permission to feel like this. Neither are you.

### Caption — Product Launch
New. And worth the wait.
...
Curated datasets for training and calibrating AI systems. Voice samples, product data, and SEO data stored here.
""",
...
Load at the start of every AI session requiring brand knowledge. Compressed AI-optimized summary of the full Brand Bible.
...
| Luxury | Show it, do not announce it | This luxury product... |
...
Before editing any critical file, copy the original here:
...
Backup protocol and recovery procedures. Active working backups are in _BACKUPS/ at root.
""",
...
### Stage 2 — Brief
Fill out visual brief template (12_Content_Assets/visual_brief_templates.md)
...
Hashtags added
Scheduled via platform scheduler
...
End-to-end content production pipeline and calendar framework.
""",
...
Technical blueprints for all major MVQUEEN systems. Plans documented here before systems are built.
""",
...
AI agent architecture, workflow protocols, and deployment standards.
""",
...
Registry of all structured data stores across the MVQUEEN ecosystem.
""",
...
A prompt chain is a sequence where each output feeds the next input.
...
Multi-step AI prompt sequences for complex creative, strategic, and analytical tasks.
""",
...
Master SOP library for all MVQUEEN operations. Every repeated task has a documented procedure.
""",
...
Permanent historical archive for files that have been superseded, restructured, or retired. Read-only.
...
4. Distinct from 28_Archives: 98 is permanent history, 28 is recent operational archive
...
Permanent historical archive. Read-only. Log all entries in archive_log.md.
""",
