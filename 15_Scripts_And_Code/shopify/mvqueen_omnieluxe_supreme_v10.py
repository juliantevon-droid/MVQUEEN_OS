...
Extends the MVQueen architecture with universal brand/persona banks,
1,000+ deterministic persona keywords, SEO enrichment, metadata,
scalable bundle-neighbor generation and Shopify safety validation.
"""
...
from mvqueen_brand_banks import (
    BRAND_NAME, BRAND_VOICE_BANK, CATEGORY_BANKS, COLLECTION_BANK,
    INTENT_PHRASES, LUXURY_ADJECTIVES, LUXURY_MODIFIERS, PERSONA_PROFILES,
    PERSONA_KEYWORDS, PROTECTED_COLUMNS, SEO_LANGUAGE_BANK, SENSORY_VERBS,
    BENEFIT_SYNONYMS, assign_brand_voice, assign_keywords, assign_persona,
    compare_at_price, price_tier, stable_seed, validate_shopify_row,
)
...
def generate_title(row: pd.Series, category: str, product_key: str) -> str:
    ...
    return clean_text(f"{prefix} {adjective} {base} — {modifier} {outcome}")[:255]
...
def localized_seo(title: str, category: str, persona: str, language: str) -> Tuple[str, str]:
    ...
    meta_description = f"{title}. A refined {beauty} ritual designed around {glow}, performance and modern {lux}. Persona: {persona}."[:320]
    if language != "en":
...
def nearest_bundle_neighbors(handles: List[str], limit: int = 20) -> Dict[str, List[str]]:
    ...
    for pos, (_, handle) in enumerate(records):
        ...
        lo, hi = max(0, pos - window), min(len(records), pos + window + 1)
...
def enrich_catalog(input_file: Path, output_dir: Path) -> Tuple[Path, Path]:
    ...
    for idx in df.index:
        ...
        if not existing:
            ...
            category = product_category(clean_text(df.at[idx, title_col]), clean_text(df.at[idx, type_col]) if type_col else "")
    ...
    fields = [
        "metafield.custom.brand_version", "metafield.custom.brand_voice", "metafield.custom.persona",
        "metafield.custom.persona_axis", "metafield.custom.persona_emotion", "metafield.custom.persona_keywords",
        "metafield.custom.category", "metafield.custom.primary_keyword", "metafield.custom.secondary_keyword",
        "metafield.custom.long_tail_keyword", "metafield.custom.luxury_modifier", "metafield.custom.luxury_adjective",
        "metafield.custom.sensory_verb", "metafield.custom.benefit_synonym", "metafield.custom.intent_phrase",
        "metafield.custom.collection", "metafield.custom.price_psychology_tier", "metafield.custom.prestige_score",
        "metafield.custom.hero_probability", "metafield.custom.routine_integration_score", "metafield.custom.search_intent_match",
        "metafield.custom.conversion_index", "metafield.custom.bundle_affinity", "metafield.custom.bundle_candidates",
        "metafield.custom.editorial_short", "metafield.custom.editorial_medium", "metafield.custom.editorial_long",
        "metafield.custom.schema_ready", "metafield.custom.search_cluster", "metafield.custom.emotional_axis",
        "metafield.custom.performance_tier", "metafield.custom.aov_influence", "compare_at_price_suggestion",
    ]
    for field in fields:
        add(field)
    ...
    for idx in df.index:
        ...
        bank = CATEGORY_BANKS.get(category, CATEGORY_BANKS["beauty"])
        primary = bank["keywords"][seed[0] % len(bank["keywords"])]
        secondary = bank["keywords"][seed[1] % len(bank["keywords"])]
        long_tail = bank["outcomes"][seed[2] % len(bank["outcomes"])]
        ...
        hero, routine, search = round((seed[1] % 101) / 100, 2), round((seed[2] % 101) / 100, 2), round((seed[3] % 101) / 100, 2)
        ...
        long = f"{title} is positioned within the MVQueen {collection} architecture. Its {persona.lower()} language combines {luxury_adjective} presentation, {sensory_verb} performance and a clear intent to {intent}."
        values = {
            "metafield.custom.brand_version": VERSION, "metafield.custom.brand_voice": voice, "metafield.custom.persona": persona,
            "metafield.custom.persona_axis": profile["axis"], "metafield.custom.persona_emotion": profile["emotions"][seed[5] % len(profile["emotions"])],
            "metafield.custom.persona_keywords": ", ".join(assign_keywords(key, 12)), "metafield.custom.category": category,
            "metafield.custom.primary_keyword": primary, "metafield.custom.secondary_keyword": secondary, "metafield.custom.long_tail_keyword": long_tail,
            "metafield.custom.luxury_modifier": luxury_modifier, "metafield.custom.luxury_adjective": luxury_adjective,
            "metafield.custom.sensory_verb": sensory_verb, "metafield.custom.benefit_synonym": benefit, "metafield.custom.intent_phrase": intent,
            "metafield.custom.collection": collection, "metafield.custom.price_psychology_tier": price_tier(price), "metafield.custom.prestige_score": prestige,
            "metafield.custom.hero_probability": hero, "metafield.custom.routine_integration_score": routine, "metafield.custom.search_intent_match": search,
            "metafield.custom.conversion_index": conversion, "metafield.custom.bundle_affinity": bundle_affinity, "metafield.custom.bundle_candidates": bundles,
            "metafield.custom.editorial_short": short, "metafield.custom.editorial_medium": medium, "metafield.custom.editorial_long": long,
            "metafield.custom.schema_ready": "true", "metafield.custom.search_cluster": f"{category}:{seed[0] % 1000:03d}",
            "metafield.custom.emotional_axis": profile["axis"], "metafield.custom.performance_tier": performance, "metafield.custom.aov_influence": aov,
            "compare_at_price_suggestion": compare_at_price(price),
        }
        for lang in languages:
            values[f"metafield.custom.seo_title_{lang}"], values[f"metafield.custom.seo_description_{lang}"] = localized_seo(title, category, persona, lang)
        for column, value in values.items():
            df.at[idx, column] = value
        if tags_col:
            existing_tags = clean_text(df.at[idx, tags_col])
            additive_tags = ", ".join([primary, secondary, persona, collection, BRAND_NAME])
            df.at[idx, tags_col] = ", ".join(dict.fromkeys(filter(None, [existing_tags, additive_tags])))
        campaign_rows.append({"handle": key, "title": title, "category": category, "persona": persona, "brand_voice": voice, "collection": collection, "price_tier": price_tier(price), "prestige_score": prestige, "conversion_index": conversion, "hero_probability": hero, "bundle_affinity": bundle_affinity})
    ...
    for idx in df.index:
        for error in validate_shopify_row({"Title": df.at[idx, title_col], "Status": df.at[idx, status_col]}):
