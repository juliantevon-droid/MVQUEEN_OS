...
(() => {
  ...
  const CATEGORY_RULES=[
    ...
    ['Women’s Fashion',/\b(dress|top|blouse|shirt|skirt|pants|trouser|jeans|jacket|coat|bodysuit|jumpsuit|romper|cardigan|sweater|hoodie|clothing|fashion|outfit)\b/i]
  ];
  function source(p={}){return [p.Title,p['Product Type'],p.Tags,p['Body HTML'],p['Short Description'],p.Description,p.Benefits,p.Usage,p.Ingredients,p.Fabric,p.Fit,p.Specifications].filter(Boolean).join(' | ');}
  function classifyProduct(p={}){const explicit=clean(p['Product Type']); if(explicit && explicit.toLowerCase()!=='other') return explicit; const s=source(p); for(const [category,re] of CATEGORY_RULES) if(re.test(s)) return category; return 'Other';}
  ...
  function sanitizeExistingHTML(html=''){
    let value=String(html??'');
    value=value.replace(/<script\b[^>]*>[\s\S]*?<\/script>/gi,'').replace(/<style\b[^>]*>[\s\S]*?<\/style>/gi,'');
    value=value.replace(/\son\w+\s*=\s*(['"]).*?\1/gi,'').replace(/\son\w+\s*=\s*[^\s>]+/gi,'').replace(/javascript:/gi,'');
    STOP.lastIndex=0; value=value.replace(STOP,'');
    return value.replace(/\s{2,}/g,' ').trim();
  }
  function buildBodyHTML(p,a){
    const n=clean(p.Title)||'MVQueen product';
    const existing=sanitizeExistingHTML(p['Body HTML']||p.Description||'');
    const facts=[];
    if(a.benefits)facts.push(`<li><strong>Benefits:</strong> ${a.benefits}</li>`);
    if(a.usage)facts.push(`<li><strong>Usage:</strong> ${a.usage}</li>`);
    if(a.ingredients)facts.push(`<li><strong>Ingredients:</strong> ${a.ingredients}</li>`);
    if(a.skin_type)facts.push(`<li><strong>Skin type:</strong> ${a.skin_type}</li>`);
    if(a.finish)facts.push(`<li><strong>Finish:</strong> ${a.finish}</li>`);
    if(a.texture)facts.push(`<li><strong>Texture:</strong> ${a.texture}</li>`);
    if(a.fabric)facts.push(`<li><strong>Fabric:</strong> ${a.fabric}</li>`);
    if(a.fit)facts.push(`<li><strong>Fit:</strong> ${a.fit}</li>`);
    if(a.silhouette)facts.push(`<li><strong>Silhouette:</strong> ${a.silhouette}</li>`);
    if(a.occasion)facts.push(`<li><strong>Occasion:</strong> ${a.occasion}</li>`);
    if(a.care)facts.push(`<li><strong>Care:</strong> ${a.care}</li>`);
  ...
  function buildTags(p,a){
    STOP.lastIndex=0;
    const raw=String(p.Tags||'').split(',').map(x=>clean(x)).filter(Boolean).filter(x=>{STOP.lastIndex=0;return !STOP.test(x);});
    return [...new Set([...raw,a.category,...a.keywords,'MVQueen'].filter(Boolean))].join(', ');
  }
  function optimizeProduct(p={}){
    const out={...p}; const category=classifyProduct(p); const a=extractAttributes(p,category); const name=clean(p.Title);
    if('Title'in out&&name)out.Title=name.toLowerCase().includes(BRAND.toLowerCase())?name:`${name} | ${BRAND}`;
    if('Product Type'in out)out['Product Type']=category;
    if('Short Description'in out)out['Short Description']=buildShortDescription(p,a);
    if('Body HTML'in out)out['Body HTML']=buildBodyHTML(p,a);
    if('Tags'in out)out.Tags=buildTags(p,a);
    if('SEO Title'in out)out['SEO Title']=out.Title||name;
    if('SEO Description'in out)out['SEO Description']=buildShortDescription(p,a).slice(0,157).replace(/\s+\S*$/,'')+'...';
    if('Image Alt Text'in out)out['Image Alt Text']=`${name} ${category.toLowerCase()} by ${BRAND}`.trim();
    return {product:out,intelligence:a};
  }
  ...
  function optimizeRows(headers,rows){const results=[];const optimized=rows.map(r=>{const input=Object.fromEntries(headers.map((h,i)=>[h,r[i]??'']));const result=optimizeProduct(input);results.push(result);return headers.map((h,i)=>PROTECTED.has(h)?(r[i]??''):(result.product[h]??''));});return {rows:optimized,results};}
  window.MVQUEENIntelligence={classifyProduct,extractAttributes,buildShortDescription,sanitizeExistingHTML,buildBodyHTML,buildTags,optimizeProduct,auditProduct,optimizeRows,PROTECTED};
})();